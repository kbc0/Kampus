-- First drop existing indexes that depend on the function
...
BEGIN
  ...
  SELECT * FROM (
    ...
    SELECT 
      ...
      NULL as content,
      NULL as author_name,
      similarity(normalize_turkish(p.name), normalize_turkish(search_query)) as name_sim,
      similarity(normalize_turkish(p.university), normalize_turkish(search_query)) as univ_sim
    FROM profiles p
    WHERE 
    ...
    SELECT 
      ...
      similarity(normalize_turkish(t.title), normalize_turkish(search_query)) as title_sim,
      similarity(normalize_turkish(t.content), normalize_turkish(search_query)) as content_sim
    FROM forum_topics t
  ...
  ORDER BY 
    GREATEST(
      COALESCE(name_sim, 0),
      COALESCE(univ_sim, 0),
      COALESCE(title_sim, 0),
      COALESCE(content_sim, 0)
    ) DESC,
    created_at DESC
  LIMIT 20;
END;
