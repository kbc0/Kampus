...
DROP FUNCTION IF EXISTS get_forum_topics(uuid, text, text);
...
BEGIN
  ...
  ORDER BY
    CASE WHEN sort_by = 'trending' THEN t.likes_count::bigint 
         ELSE extract(epoch from t.created_at)::bigint 
    END DESC,
    t.created_at DESC;
END;
